...
class TrainingPipeline:
    ...
    def run(self) -> dict:
        ...
        try:
            ...
            # ---------- 🔥 FIX 1: MRD ke liye OHLCV WAPAS ADD KARO ----------
            ...
            # ---------- 🔥 NEW: Regime Detector ke missing features compute karo ----------
            # Log returns
            ...
            # ADX and RSI using pandas_ta (if available)
            if TA_AVAILABLE:
            ...
            # ---------- 🔥 FIX 2: Validation ke liye bhi OHLCV WAPAS ADD KARO ----------
            for col in ['open', 'high', 'low', 'close', 'volume']:
            ...
            # ---------- 🔥 NEW: Validation ke liye bhi missing features compute karo ----------
            ...
            regime_detector.fit(df_train_feats)  # Ab isme sab features hain
            ...
            # ---------- 🔥 FIX 3: OHLCV KO PERMANENTLY DF MEIN RAKHO, LEKIN FEATURE LIST SE HATAO ----------
            ohlcv_cols = ['open', 'high', 'low', 'close', 'volume']
            protected_cols = ['timestamp', self.config.get('target_col', 'target')]
            
            # final_features: exclude OHLCV and protected
            final_features = [c for c in df_train_feats.columns if c not in ohlcv_cols and c not in protected_cols]
            ...
            logger.info(f"   🔥 (OHLCV columns kept in dataframe for target/env, but BLOCKED from model features)")
