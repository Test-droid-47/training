#!/usr/bin/env python3
"""
Part 2 – Model Training Pipeline (Standalone)
...
- Dynamic feature tracking: only FeatureEngine outputs go to BorutaSHAP
- Alpha, SMC, Regime features are always kept (external)
- Strict separation: final_features (only inputs) vs df_cols_to_keep (includes target/timestamp)
- Leakage‑proof scaling for LSTM (validation uses training scaler, never refit)
...
- Saves two lists in final_features.json:
    - 'final_features': features used by LSTM, Ensemble, PPO (scaled inputs)
    - 'regime_features': the 7 features used by MarketRegimeDetector
...
from brain_manager import FeatureBrain
...
class TrainingPipeline:
    ...
    def train_ppo(self, df_train: pd.DataFrame, pred_model: PredictionModel, feature_cols: List[str]) -> Optional[PPOAgent]:
        ...
        if 'close' not in feature_cols:
            logger.error("'close' feature missing from final feature list")
            return None
    ...
    def save_artifacts(self, pred_model: PredictionModel, ensemble: EnsembleModel,
        ...
        # Save final_features.json with two lists
    ...
    def run(self) -> dict:
        ...
        logger.info("=" * 70)
        logger.info("PART 2 – AUTOMATED TEMPORAL DATA ROUTING PIPELINE")
        logger.info("=" * 70)
        ...
        try:
            ...
            logger.info("Building features on isolated Training Split...")

            df_train_feats = FeatureEngine.build_all(df_train_raw.copy())
            protected_infrastructure = ['timestamp', 'open', 'high', 'low', 'close', 'volume',
                                        self.config.get('target_col', 'target')]
            base_cols = [c for c in df_train_feats.columns if c not in protected_infrastructure]
            logger.info(f"🎯 Identified {len(base_cols)} Pure Base Features from feature_engine.py for BorutaSHAP.")

            df_train_feats = AlphaFactorEngine(self.config).build_all(df_train_feats)
            df_train_feats = SmartMoneyEngine(self.config).build_all(df_train_feats)
            ...
            df_val_feats = AlphaFactorEngine(self.config).build_all(df_val_feats)
            df_val_feats = SmartMoneyEngine(self.config).build_all(df_val_feats)
            ...
            # Extract the exact regime features used by the detector (from its config)
            regime_features = self.config.get('regime_features',
                ['log_ret_20', 'log_ret_5', 'natr', 'adx', 'hurst_exp', 'vol_ratio', 'rsi'])
            logger.info(f"Regime features (hardcoded / from config): {regime_features}")

            all_cols = list(df_train_feats.columns)
            external_cols = [c for c in all_cols if c not in base_cols and c not in protected_infrastructure]
            self.stats['total_features'] = len(base_cols) + len(external_cols)
            self.stats['external_features'] = len(external_cols)
            logger.info(f"External features (alpha, smc, regime): {len(external_cols)}")
            ...
            # ----------------------------------------------------------------
            final_features = selected_base + external_cols
            for price_col in ['open', 'high', 'low', 'close', 'volume']:
                if price_col in df_train_feats.columns and price_col not in final_features:
                    final_features.append(price_col)

            self.stats['final_features'] = len(final_features)
            self.stats['selected_base_features'] = len(selected_base)
            logger.info(f"Final Input Features Schema size: {len(final_features)} ({len(selected_base)} base kept)")

            # ----------------------------------------------------------------
            # STEP 8: CREATE DATAFRAME SLICE LIST (Features + Infrastructure)
            # ----------------------------------------------------------------
            ...
            # Clean and drop history buffer from validation using the safe DataFrame slice
            ...
            logger.info(f"Total features built: {self.stats['total_features']}")
            logger.info(f"Selected base features: {self.stats['selected_base_features']}")
            logger.info(f"External features kept: {self.stats['external_features']}")
            logger.info(f"Final features: {self.stats['final_features']}")
