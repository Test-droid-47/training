...
class PredictionModel:
    ...
    def build(self, input_shape: Tuple[int, int]) -> Model:
        ...
        # ============================================================
        # 🔥 FIX (Us Banday ka): LOSS WEIGHTS BALANCED
        #   Direction (1.08) ko 0.5 se giraao.
        #   Price (0.00005) ko 10000 se uthaao.
        #   Baaki regression heads ko 10x/100x karo.
        # ============================================================
        model.compile(
    ...
    def _engineer_targets(df: pd.DataFrame, close_scaled: np.ndarray, max_exit_bars: int = 10):
        ...
        for i in range(n-1):
            ...
            # 🔥 Relaxed Threshold (Meri last file wala)
            threshold = max(atr_pct * 0.15, 0.0005)
    ...
    def train(self, X_train, X_val, y_train, y_val):
        ...
        # ============================================================
        # 🔥 FIX (Us Banday ka): ReduceLROnPlateau RELAXED
        #   Patience 12 (7 se zyada), min_lr 1e-4 (1e-6 se zyada)
        # ============================================================
        callbacks = [
            ...
            ReduceLROnPlateau(
                monitor='val_loss',
                factor=0.5,
                patience=12,        # Pehle 7 tha → 12 kiya
                min_lr=1e-4,        # Pehle 1e-6 tha → 1e-4 kiya
                verbose=0
            ),
        ]
        self.model.fit(
