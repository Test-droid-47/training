...
class PredictionModel:
    ...
    def build(self, input_shape: Tuple[int, int]) -> Model:
        ...
        # 🔥 FIX: Architecture Halka (1 Conv → 1 LSTM → 1 Transformer)
        ...
        # Sirf 1 Transformer block (pehle 2 the)
        ...
        # ==============================================================
        # 🔥 FIX: AdamW + Cosine Decay (Us Banda #8, #9)
        # 🔥 FIX: Label Smoothing (Us Banda #16)
        # 🔥 FIX: Balanced Loss Weights (Us Banda #1)
        # 🔥 FIX: clipnorm HATAO (Mera Fix)
        # ==============================================================
        total_steps = self.cfg.get('epochs', 100) * 522  # ~522 batches per epoch
        lr_schedule = CosineDecay(
            initial_learning_rate=self.cfg.get('learning_rate', 0.001),
            decay_steps=total_steps,
            alpha=1e-4  # minimum learning rate
        )
    ...
    def _engineer_targets(df: pd.DataFrame, close_scaled: np.ndarray, max_exit_bars: int = 10):
        ...
        # Multi-Horizon Price Target (40% next, 30% 3-bar, 20% 5-bar, 10% 10-bar)
        y_price = np.zeros(n-1, dtype=np.float32)
        ...
        for i in range(n-1):
            ...
            # Weighted smooth return
            smooth_ret = 0.4 * ret1 + 0.3 * ret3 + 0.2 * ret5 + 0.1 * ret10
            y_price[i] = float(smooth_ret)  # Scale later
            
            # Direction based on smoothed return
            atr_pct = atrs[i] / (closes[i] + 1e-10)
            threshold = max(atr_pct * 0.3, 0.0008)  # Relaxed threshold
            if smooth_ret > threshold:
                y_direction[i] = 2
            elif smooth_ret < -threshold:
                y_direction[i] = 0
            # else remains 1 (HOLD)
    ...
    def prepare_data(self, df: pd.DataFrame, feature_cols: List[str] = None):
        ...
        # 🔥 FIX: Model Indices se CLOSE ko EXCLUDE karo (Index Mismatch ka permanent solution)
        ...
        cont_cols = [c for c in all_cont_cols if c != 'close']  # close hata diya
        ...
        if self.scaler is None:
            ...
            self.scaler.fit(train_data[all_cont_cols])  # scale close bhi, lekin model ko nahi milegi
        ...
        self._feature_cols = [c for c in feature_cols if c != 'close']  # close hata di
    ...
    def train(self, X_train, X_val, y_train, y_val):
        ...
        # OOM fix: use predict with small batch size
