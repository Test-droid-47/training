...
class PredictionModel:
    ...
    def build(self, input_shape: Tuple[int, int]) -> Model:
        ...
        model.compile(
            ...
            loss_weights={
                'price_pred': 0.5,
                'direction': 1.0,           # FIX: 2.0 se 1.0 kiya (taake dominate na kare)
                'entry_quality': 1.5,
                'exit_bar': 0.8,
                'position_size': 0.8
            },
    ...
    def _engineer_targets(df: pd.DataFrame, close_scaled: np.ndarray, max_exit_bars: int = 10):
        ...
        for i in range(n-1):
            ...
            # FIXED THRESHOLD: 0.5 → 0.15, 0.003 → 0.0005
    ...
    def train(self, X_train, X_val, y_train, y_val):
        ...
        # OOM fix: use predict with small batch size instead of _call_model
